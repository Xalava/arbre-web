(function($, ko){
	...
	(function(ns) {
		...
		var TalentTree = ns.TalentTree = function(_e){
			...
			//Avatar properties
			self.avatarName = ko.observable('Ton nom');
			...
			self.portraitURL = ko.computed(function(){
				return 'img/portraits/' + self.portrait() + '.jpg';
			});
		...
		var Skill = ns.Skill = function(_e, allSkills){
			...
			//Basic properties
			self.id = e.id || 0;
			self.title = e.title || 'Compétence inconnue';
			...
			self.helpMessage = ko.computed(function(){
				if(!self.dependenciesFulfilled()){
					...
					return 'Apprendre ' + prettyJoin(s) + ' pour debloquer.'
				} else if (self.canAddPoints()) {
					return 'Cliquer pour ajouter un point!';
				}
				return '';
			});
			self.talentSummary = ko.computed(function(){
		...
		var Link = ns.Link = function(_e){
			...
			//Basic properties
			self.label = e.label || (e.url || 'En savoir plus');
